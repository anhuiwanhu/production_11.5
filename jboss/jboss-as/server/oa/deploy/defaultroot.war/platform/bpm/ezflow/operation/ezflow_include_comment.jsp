...
%>
<!---------显示已经保存的批示意见内容---------->
<table width="100%" border="0"   cellpadding="0" cellspacing="0" align="center" <%if(!gd.equals("1")&&include_p_wf_moduleId.equals("1")){%>style="margin-top:-5px;"<%}%> >
  <%
    String activityId="";
	String activityName="";
   ...
   <tr id="commTR" style="display:<%=displayTr?"":"none"%>">
      <td>
         <table border="0" width="100%" <%if(j==0 && tmpList.size()>1){out.print("style=\"border:1px dashed #C6CCD2;border-collapse:collapse;\"");}else if(j==tmpList.size()-1){out.print("style=\"border:1px dashed #C6CCD2;border-collapse:collapse;\"");}else{out.print("style=\"border:1px dashed #C6CCD2;border-collapse:collapse;\"");}%> style="margin-bottom:5px;"   cellpadding="0">
		    <tr>
			 <!---此td显示活动名---->
             <td width="180"   align="center" valign="middle" style="border-bottom:<%=j==(tmpList.size()-1)?"1":"0"%>px dashed #C6CCD2; font-weight:bold; border-right:1px dashed #C6CCD2;"><%=activityName%>：</td>
			       ...
			       <%}else{
					   ...
					   if(commentType.equals("2")){
						...
						if(commentType.equals("0")){
							String realdealUserName=ub.getSignature(dealUserId).equals("")?dealUserName:("<IMG SRC='"+(smartInUse_w==1?rootPath:include_fileServer)+"/upload/peopleinfo/"+ub.getSignature(dealUserId)+"'>");
							  //<!--办理人 以及 转办 代办--->
						      jsContent+="<div style=\\\"width:100%;text-align:right\\\">"+((commentStatus==2&&!"0".equals(""+request.getAttribute("p_wf_processType")))?"转办人：":"")+realdealUserName+" "+dealOrgName+" "+daalTimeStr+((isStandFor==1&&!standForUserName.equals("")&&!standForUserName.equals(""))?"<font color=\\\"blue\\\">(代"+standForUserName+"办理)</font>":"")+""+"<br></div>";
						}else{
						      jsContent+="<div style=\\\"width:100%;text-align:right\\\">"+((commentStatus==2&&!"0".equals(""+request.getAttribute("p_wf_processType")))?"转办人：":"")+dealUserName+" "+dealOrgName+" "+daalTimeStr+""+"<br></div>";
