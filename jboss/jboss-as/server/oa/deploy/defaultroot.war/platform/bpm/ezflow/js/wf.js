...
//显示留言
function  showWorkFlowMessage(id){
	var processId =$('#p_wf_processId').val();
	var processInstanceId =$('#p_wf_processInstanceId').val();
	var recordId =$('#p_wf_recordId').val();
	var activityName =$('#p_wf_cur_activityName').val();
	var remindTitle =$('#p_wf_remindTitle').val();
	var openType =$('#p_wf_openType').val();
	if($("#"+id).html()==""||recordId!=""){
		var url=whirRootPath+"/platform/bpm/pool/pool_flowmessage.jsp?openType="+openType+"&processId="+processId+"&processInstanceId="+processInstanceId+"&recordId="+recordId+"&activityName="+encodeURIComponent(activityName)+"&remindTitle="+encodeURIComponent(remindTitle);
		//alert(url);
		var html = $.ajax({url: url,async: false,cache:false}).responseText;
		//alert(html);
		$("#"+id).html(html);
...
//显示流程跟踪单
function showWorkFlowTrackingList(id){
	var url="/defaultroot/platform/bpm/ezflow/operation/ezflow_trackingList.jsp?p_wf_processInstanceId="+$("#p_wf_processInstanceId").val();
	var html = $.ajax({url: url,async: false,cache:false}).responseText;
	$("#"+id).html(html);
}
		...
		$.dialog.tips(workflowMessage_js.dealsuccess,1.5,'success.gif',function(){
			...
			if(p_wf_saveType_v=="0"){
				...
				if(data.gdInfo!=null&&data.gdInfo!=""){
					...
					if(data.openHref !=null&&data.openHref!=""){
						//openWin({url:data.openHref,isFull:true,winName:'openWorkFlow'});
						location_href(data.openHref);
					}else{
						window.close();
					}
				} 
...
//按钮强制结束
function include_CompulsoryEndSubmit(){
	var p_wf_processInstanceId =$('#p_wf_processInstanceId').val();
	var url=whirRootPath+"/EzFLOWDealSearch!deleteWorkLog.action?status=100&p_wf_processInstanceId="+p_wf_processInstanceId; 
	//alert(url);
	$("#dataForm").attr("action",url);
	setCallBackName("");
	$("#dataForm").submit(); 
