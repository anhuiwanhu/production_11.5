...
boolean delRight=false;
if(managerBD.hasRight(session.getAttribute("userId").toString(), "02*03*01")){
	delRight =true;
}
%>
<html>
  ...
  <%
	  ...
	  %>
		  <tr  id="TR_<%=pId%>">
			...
			<td style="display:" class="subTitleList">
			<%if(openType!=null && (openType.equals("startOpen") || openType.equals("waitingDeal") || openType.equals("fromDraft") || openType.equals("reStart") || openType.equals("startAgain") || openType.equals("modifyView"))){%>
			<%
			if((delRight && openType.equals("modifyView")) || createUserId.equals(session.getAttribute("userId").toString())){
			%>
			<img src="<%=rootPath%>/images/del.gif" onclick="delRelationWork('<%=realtionId%>');" style="cursor:hand" title="<bean:message bundle="common" key="comm.sdel"/>">
			<%}}%>&nbsp;
