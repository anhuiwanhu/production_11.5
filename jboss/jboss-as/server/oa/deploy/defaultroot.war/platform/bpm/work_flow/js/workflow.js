...
function  cmdSend(){
	...
	if(boradroomId!=null && boradroomId!="" && boradroomId!=undefined){
		...
		//发起流程时重置p_wf_now_dealactivityClass的值为0 当为4时 表明是 分叉活动选择
		$("#p_wf_now_dealactivityClass").val("0");
	...
	$.ajax({
		...
		success:function(dataForm){
			...
			if(flag=="0"){//走流程
				...
				//保存数据
				$("#dataForm").attr("action",whirRootPath+"/boardRoom!saveOldApprovalData.action");
				$("#dataForm").submit();
				whir_alert("保存成功！");
