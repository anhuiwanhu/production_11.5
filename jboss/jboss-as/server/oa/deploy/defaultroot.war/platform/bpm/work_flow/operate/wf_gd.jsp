...
if(request.getParameter("gd") != null) {
	...
	String p_submitPerson = request.getParameter("p_submitPerson")!=null?request.getParameter("p_submitPerson"):"";
	String p_submitTime = request.getParameter("p_submitTime")!=null?request.getParameter("p_submitTime"):"";
...
//    String fileName = application.getRealPath(request.getRequestURI()).split("defaultroot")[0]+"defaultroot\\archivesfile\\" + tmp;
    ...
    gdBD.gdDossier(workId,"GZLC",p_submitPerson + " " + p_submitTime + " 的" + title,null,null,null,session.getAttribute("orgName")+"",now.get(Calendar.YEAR)+"",null,null,"",null,createEmp,createOrg,sdf.format(gdDate),session.getAttribute("domainId").toString(),"htm",tmp,null,null,null);
	...
	rd.write("<html><head><title>流程归档</title><meta http-equiv='Content-Type' content='text/html; charset=UTF-8' /><script src='/defaultroot/scripts/jquery-1.11.2.min.js' type='text/javascript'></script><script src='/defaultroot/scripts/i18n/zh_cn/CommonResource.js' type='text/javascript'></script><link   href='/defaultroot/themes/2012/blue/style.css' rel='stylesheet' type='text/css'/><style type=\"text/css\">.doc_content{ padding:20px; line-height:100%; height:100%; overflow:visible;} .doc_width{ width:96%; margin:0px auto; }  .docbox_noline{padding:0px 10px;  min-height:100%;   height:100%;  } .doc_content{ padding:20px; line-height:100%;width:98%} </style><SCRIPT LANGUAGE=\"JavaScript\">var whirRootPath = \""+com.whir.component.config.PropertiesUtil.getInstance().getRootPath()+"\"; function  readyFun(){}       function initFormFunc(){} </SCRIPT></head>");
