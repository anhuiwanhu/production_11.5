...
MetaNode.prototype.editActivity = function () {
	...
	if(this.getModel().type==MetaNodeModel.TYPE_END_NODE){
	 ...
	 url = CommonJSResource.rootPath+ "/ezflowprocess!setActivity.action?processKey="+_xiorkFlowModel.getID()+"&id="+model.getID()+"&formCode="+_xiorkFlowModel.getAttribute("whir:formKey")+"&moduleId="+moduleId ;
...
MetaNode.prototype._updatePosition = function () {
	...
	this.wrapper.getViewer().updateSvgPosition(point, this.model.getSize());
};
MetaNode.prototype._updateSize = function () {
    ...
    this.wrapper.getViewer().updateSvgPosition(this.model.getPosition(), size);
};
...
MetaNode.prototype._updateClassName = function () {
	...
	if(className=="buttonface"){
		...
		if(this.model.judgeIsEvent()){
			var vimg=XiorkFlowWorkSpace.XIORK_FLOW_PATH+"/src/name/xio/xiorkflow/beginended.png";
			this.getUI().style.background= "url(' " + vimg + "') no-repeat center 50%";  
		}else if(this.model.judgeIsGate()){
