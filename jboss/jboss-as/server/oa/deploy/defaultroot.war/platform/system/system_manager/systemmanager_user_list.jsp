...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="SearchBar">    
    ...
    <tr>  
        ...
        <td class="whir_td_searchtitle">企业号：</td>  
        <td class="whir_td_searchinput">
        ...
        <td class="whir_td_searchtitle">移动办公：</td>  
        <td class="whir_td_searchinput">
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" class="listTable">  
    ...
    <tr class="listTableHead">  
        ...
        <s:if test="status != 'apply'">
        <td whir-options="field:'enterprisenumber',width:'3%',renderer:renderEnterprisenumber">企业号</td>
        <td whir-options="field:'mobileUserFlag',width:'5%',renderer:renderMobileUserFlag">移动办公</td>
...
function renderOperate(po,i){  
    var html = '';
    <s:if test="status == 'active'">
        html += '<img src="<%=rootPath%>/images/modi.gif" title="修改" onClick="javascript:openWin({url:\'<%=rootPath%>/User!loadUser.action?empId='+po.empId+'&verifyCode='+po.verifyCode+'\',width:620,height:350,winName:\'modifyUser\',isFull:true,isPost:false});"><img src="<%=rootPath%>/images/del.gif" title="删除" onClick="javascript:deleteMe(\''+po.empId+'\',\''+po.verifyCode+'\');"><img src="<%=rootPath%>/images/stop.gif" title="禁用" style="cursor:hand" onClick="javascript:disableMe('+po.empId+',\''+po.verifyCode+'\');">';
...
//禁用用户
function disableMe(id,verifyCode){
    ajaxOperate({urlWithData:"<%=rootPath%>/User!disableUser.action?empId="+id+"&verifyCode="+verifyCode,tip:"<s:text name="comm.disabled2"/>",isconfirm:true,formId:"queryForm",callbackfunction:null});
}
