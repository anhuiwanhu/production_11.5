...
<%
  ...
  Long trigId=po.getTrigSettingId()==null?-1:(Long)po.getTrigSettingId();
  String tableModelId = (String)request.getAttribute("tableModelId");
  List navMenuList = (List)request.getAttribute("navMenuList");
  String vaid_s=request.getParameter("validate")==null?"":com.whir.component.security.crypto.EncryptUtil.htmlcode(request.getParameter("validate"));
  if(null!= vaid_s && !"".equals(vaid_s) && !"null".equals(vaid_s)){
	vaid_s = com.whir.component.security.crypto.EncryptUtil.replaceHtmlcode(vaid_s);
  }
  String id_s=request.getParameter("id")==null?"":com.whir.component.security.crypto.EncryptUtil.htmlcode(request.getParameter("id"));
%>
<head>  
	<title>修改自定义模块</title>  
...
<body class="Pupwin" >  
	<div class="BodyMargin_10">    
		...
		<table width="100%" border="0" cellpadding="2" cellspacing="0" class="Table_bottomline"> 
		   ...
		   <input name="trigId" type="hidden" id="trigId" value="<%=trigId%>">
		   <tr>    
		   ...
		   <tr>    
			   <td width="100" class="td_lefttitle" for="主菜单排序码">    
			   ...
			   <td>
				  ...
				  <div id="mainmenuorder" style="display:inline;">
                     主菜单排序码：<input class="inputText"  name="parentOrder" id="parentOrder" style="width:150px;" whir-options="vtype:['p_integer_e',{'range':'0-9999'}]" maxlength="9" value="<%=po.getMenuOrderSet()==null?"":po.getMenuOrderSet()%>">
			     </div>
			   </td>    
		   </tr>
		  ...
		  <tr id="mobileEnterpriseId" <%if("-1".equals(po.getMenuBlone()+"")){%>style="display:none"<%}else{%>style="display:none;"<%}%>>    
		  ...
		  <tr id="linkTR2" id style="display:none">
			...
			<td valign="top" align="left" style="padding: 6px 2px 6px 45px;">
				...
				<table>
					 ...
					 <!--字段联动-->
					 <tr id="tr_relationTrig" style="display:none">
						  <td valign="top">字段联动：&nbsp;&nbsp;&nbsp;
						  <select name="relationTrig" id="relationTrig" style="width:200px;"><option value="-1">--</option>
						  </select>
						  </td>	
                          <td>&nbsp;</td>						 
					 </tr>  
...
function resetDataForm(){
  location_href('<%=rootPath%>/custormermenu!modiCustMenu.action?validate=<%=vaid_s%>&id=<%=id_s%>');
}
