<%@ taglib uri="/WEB-INF/tag-lib/struts-tiles.tld" prefix="tiles" %><%@ taglib uri="/WEB-INF/tag-lib/struts-nested.tld" prefix="nested" %><%@ taglib uri="/WEB-INF/tag-lib/struts-logic.tld" prefix="logic" %><%@ taglib uri="/WEB-INF/tag-lib/struts-template.tld" prefix="template" %><%@ taglib uri="/WEB-INF/tag-lib/struts-bean.tld" prefix="bean" %><%@ taglib uri="/WEB-INF/tag-lib/struts-html.tld" prefix="html" %><%@ page import="java.util.*"%><%@ page import="com.whir.component.security.crypto.EncryptUtil"%><%@ page import="java.text.*"%><%
	...
	String moduleType=request.getParameter("moduleType")==null?"default":EncryptUtil.replaceHtmlcode(request.getParameter("moduleType"));
...
<%
	...
	List list = newInformationBD.getDataBySQL("select copyEnable,COPYENABLEVIEW from  gov_senddocumentbaseinfo ");
	String copyEnable = "0";
...
<%
 ...
 // String mUserName= com.whir.component.security.crypto.EncryptUtil.replaceHtmlcode(request.getParameter("UserName")) ;//EncryptUtil.htmlcode( request.getParameter("UserName") );
  String mUserName=session.getAttribute("userName")+"";
  if(request.getParameter("UserName")!=null&&!request.getParameter("UserName").toString().equals("")&&!request.getParameter("UserName").toString().equals("null")){
	   mUserName= com.whir.component.security.crypto.EncryptUtil.replaceHtmlcode(request.getParameter("UserName")) ;
  }

  String textContent= com.whir.component.security.crypto.EncryptUtil.htmlcode(request.getParameter("textContent"));
  ...
  String isSaveDocFile=request.getParameter("saveDocFile")==null?"0":EncryptUtil.replaceHtmlcode(request.getParameter("saveDocFile"));//是否生成DOC文件
  ...
  //zhuo add 是否显示红头和签章
  //签章
  String showTempSign = request.getParameter("showTempSign")==null?"0":EncryptUtil.htmlcode(request.getParameter("showTempSign"));

  //红头
  String showTempHead=request.getParameter("showTempHead")==null?"0":EncryptUtil.htmlcode(request.getParameter("showTempHead"));

  String  isShowSign=request.getParameter("ShowSign")==null?"":EncryptUtil.htmlcode(request.getParameter("ShowSign"));

  //显示痕迹按钮
  String showSignButton=request.getParameter("showSignButton")==null?"0":EncryptUtil.htmlcode(request.getParameter("showSignButton"));

  //显示编辑按钮
  String showEditButton=request.getParameter("showEditButton")==null?"0":EncryptUtil.htmlcode(request.getParameter("showEditButton"));

  //显示套打按钮
  String showCoverPrint=request.getParameter("showCoverPrint")==null?"1":EncryptUtil.htmlcode(request.getParameter("showCoverPrint"));

  //显示转PDF按钮
  String showTransPDF=request.getParameter("showTransPDF")==null?"0":EncryptUtil.htmlcode(request.getParameter("showTransPDF"));

  //1:起草正文 2：批阅正文 3：生成正式文件  0：查看正文
  String openWordType=request.getParameter("openWordType")==null?"0":EncryptUtil.htmlcode(request.getParameter("openWordType"));

  // 默认不是第一次进; 当为是第一次进时， isFirstIn:1
  String isFirstIn="-1";
  // 是否 打印按扭
  String isPrintButton=request.getParameter("printButton")==null?"0":EncryptUtil.htmlcode(request.getParameter("printButton"));

  //是否 重新加 模板  0不需要 ，1：需要
  String isReTem="0";

  //是否是查看历史痕迹版本
  String isViewOld=request.getParameter("isViewOld")==null?"0":EncryptUtil.htmlcode(request.getParameter("isViewOld"));


  String _rowIndex=request.getParameter("_rowIndex")==null?"0":EncryptUtil.htmlcode(request.getParameter("_rowIndex"));

  String copyType=request.getParameter("copyType")==null?"":EncryptUtil.htmlcode(request.getParameter("copyType"));
  ...
  if ( DbaObj.OpenConnection())
  {
    ...
    PreparedStatement pstmt = DbaObj.Conn.prepareStatement(mSql);
    //String mSql="Select * From Document Where RecordID=  '"+ mRecordID + "'";
    pstmt.setString(1,mRecordID);
    try
     {  
		if(!mRecordID.equals("")){
...
%>
  ...
  <%
	String initRecordId=request.getParameter("initRecordId")==null?"":EncryptUtil.htmlcode(request.getParameter("initRecordId"));


    if(newRecordId && initRecordId != null && !initRecordId.equals("")){%>
         webform.WebOffice.RecordID="<%=initRecordId%>";   //RecordID:本文档记录编号
   <%}	  
  %>
...
function TestWebSavePDF(){
	try{
        ...
        webform.WebOffice.WebSetMsgByName("moduleType","govdocument")
...
function LoadBookmarks(){
  StatusMsg("正在填充模扳...");
    ...
    <%if(textContent != null && !"-1".equals(textContent)){%>
      webform.WebOffice.WebSetBookMarks("Content","<%=textContent.replaceAll("\r\n","\\\\r\\\\n")%>");
  <%}%>
...
<body bgcolor="#ffffff" onLoad="Load();WebEnableCopy(true);" onUnload="UnLoad()">  <!--引导和退出iWebOffice-->
  ...
  <form name="webform" id="webform" method="post" action="DocumentSave.jsp?field=<%=field%>&_rowIndex=<%=_rowIndex%>" onSubmit="return SaveDocument('<%=moduleType%>','<%=isSaveHtmlImage%>','<%=isSaveDocFile%>','<%=isSaveHtml%>');" target="targetFrame">  <!--保存iWebOffice后提交表单信息-->
