...
String showUnitOrg = request.getParameter("showUnitOrg")!=null?com.whir.component.security.crypto.EncryptUtil.replaceHtmlcode(request.getParameter("showUnitOrg")):"1";//默认显示
String showPersonalGroup = com.whir.component.security.crypto.EncryptUtil.replaceHtmlcode(request.getParameter("showPersonalGroup"));
String tagitId = request.getParameter("tagitId")!=null?com.whir.component.security.crypto.EncryptUtil.replaceHtmlcode(request.getParameter("tagitId")):"";
String range = com.whir.component.security.crypto.EncryptUtil.replaceHtmlcode((String)request.getAttribute("range"));
String selfUnitId = com.whir.component.security.crypto.EncryptUtil.replaceHtmlcode((String)request.getAttribute("selfUnitId"));
ConfigXMLReader reader = new ConfigXMLReader();
String orgTreeSetting_expand = reader.getAttribute("OrgTreeSetting", "expand");
String ogp = request.getParameter("ogp")!=null?com.whir.component.security.crypto.EncryptUtil.replaceHtmlcode(request.getParameter("ogp")):"";
if(ogp.toLowerCase().indexOf("alert(")!=-1 || ogp.indexOf(" ")!=-1 || ogp.indexOf("\"")!=-1){
    ogp = "";
}
String showShortcut = request.getParameter("showShortcut")!=null?com.whir.component.security.crypto.EncryptUtil.replaceHtmlcode(request.getParameter("showShortcut")):"";

String layer = request.getParameter("layer")==null?"":com.whir.component.security.crypto.EncryptUtil.htmlcode(request.getParameter("layer"));//yes-layer弹出框,no-dialog弹出框
...
<%if(isShowUnitOrg){%>
                        ...
                        var settingOrg_selfUnit = {
                            ...
                            async: {
                                ...
                                url:"<%=rootPath%>/SelectOrgAndUser!filterOrgTree.action",
                                autoParam:["parentOrgId"],
                                otherParam:{"select":$('#select').val(), "single":$('#single').val(), "show":$('#show').val(), "range":$('#range').val(), "currentOrgId":$('currentOrgId').val(), "limited":$('#limited').val(), "fromModule":$('#fromModule').val(), "key":$('#key').val(), "selfUnit":"1", "selfUnitId":"<%=selfUnitId%>"}
                            },
                            callback: {
...
function okSelected(){
	...
	//alert(ret_selectedName);
	//ret_selectedName = ret_selectedName.substring(0,ret_selectedName.length-1);

    W.document.getElementById(p_allowId).value   = ret_selectedId;
	//alert(p_allowName);
    W.document.getElementById(p_allowName).value = ret_selectedName;
	//alert(W.document.getElementById(p_allowName).value);
    //回调
    if($('#callbackOK').val()!=''){
        try{
            var cb = eval('W.'+$('#callbackOK').val());
			cb.call(this);
        }catch(e){}
    }
    ...
    %>
        //回调tagit
        W.callbackTagit({tagitId:'<%=com.whir.component.security.crypto.EncryptUtil.replaceHtmlcode(tagitId)%>', inputId:p_allowId, inputName:p_allowName, single:<%if("yes".equals(single)){%>true<%}else{%>false<%}%>});
    <%}%>
