...
#div_<%=uniqueId%>{width:<%=width%>px;height:<%=height%>px;cursor:pointer;cursor:hand;text-align:center;background-color:transparent;<%if(buttonImage.equals("null")){ %>border:1px solid gray;<%}%>padding-top:3px;margin-top:5px;}
...
<%=com.whir.component.security.crypto.EncryptUtil.htmlcode(style)%>
-->
</style>
<div id="<%=uniqueId%>" >
...
$(document).ready(function() {
	...
	if(_lochref.indexOf('/archivesfile/')!=-1 && _lochref.indexOf('/archivesfile/GW_')==-1){//<%--档案系统文件--%>
		//2017-3-1 luosong 修改合同归档问题 做特殊处理
		if(document.getElementById("p_wf_moduleId")){
			if($("#p_wf_moduleId").val()=="92" || $("#p_wf_moduleId").val()=="93"){ 
				//不做处理
			}else{
				return;
			}
	...
	var uniqueId = '<%=EncryptUtil.htmlcode(uniqueId)%>';
	...
	if('<%=canModify%>' == 'yes'){
		/**-- --plupload---- --*/
		 ...
		 var uploadLimit = <%=EncryptUtil.htmlcode(uploadLimit)%>;
	     ...
	     window.up_<%=EncryptUtil.htmlcode(uniqueId)%> = uploader;
                ...
                if(typeof(W) != "undefined"){
		         ...
		         $.each(files, function(i, file) { 
		             ...
		             if(file.size==0){
						...
						whir_alert("文件名不能包含有特殊字符\',\’等",function(){up.refresh();},myapi);
						b = true;
		             	up.removeFile(file);
					}else{
