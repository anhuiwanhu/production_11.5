...
if(request.getParameter("gd") != null){
   ...
   // int ep = pageContent.indexOf("</SCRIPT>", fp);
	//System.out.println("11111111111111111111111111111111111"+fp+"::"+ep);
   // pageContent = pageContent.substring(0, fp) + pageContent.substring(ep + 9);
    ...
    String tmp = "GW_" + request.getParameter("fileId") + "_" + now.get(Calendar.YEAR) + (now.get(Calendar.MONTH) + 1) + now.get(Calendar.DATE) + ".htm";
...
//java.util.Map sysMap = com.whir.org.common.util.
//                            SysSetupReader.getInstance().
//                            getSysSetupMap(request.getSession(true).
//                            getAttribute("domainId").toString());
//		if (sysMap != null && sysMap.get("附件上传") != null &&sysMap.get("附件上传").toString().equals("0")) {
//			java.util.Map ftpMap = com.whir.component.config.ConfigReader.getUploadMap(request.getRemoteAddr(), request.
//                                    getSession(true).getAttribute("domainId").
//                                    toString());
//			//com.whir.govezoffice.documentmanager.common.util.FtpClient ftpClient = new com.whir.govezoffice.documentmanager.common.util.FtpClient();
...
<%    
	 Date date=new Date();
     String dateString1=""+(date.getYear()+1900);//+"/"+(date.getMonth()+1)+"/"+date.getDate();//request.getParameter("dateTime")
	 String dateString2=""+(date.getYear()+1900)+"/"+(date.getMonth()+1)+"/"+date.getDate();//request.getParameter("dateTime")
	 ...
	 String uname = ubd.getUserNameById(createdEmp);
		 Boolean b=dossierBD.gdDossier(request.getParameter("fileId"),"SWGL",request.getParameter("fileTitle"),request.getParameter("wh"),null,null,null,dateString1,null,null,uname,null,createdEmp,createdOrg,dateString2,session.getAttribute("domainId").toString(),"htm",tmp,null,null,null);
%>
