...
function initPara(){
        ...
        if(fieldDataObjeLength>0){
            $.each(fieldDataObj.fieldData,function(i,item){
                ...
                if($("*[name='"+fieldCodeData+"']").length>0 && $("*[name='"+fieldCodeData+"']")[0].value==""){
                    fieldFlag="false";
                    whir_alert(item.fieldNameData+"不能为空!",function(){$("*[name='"+fieldCodeData+"']").focus()});
                    return false;
                }
            });
        }
    ...
    *//**
     * 针对11.5
     *//*
    if( $("input[type='hidden'][name='receiveFileSendFileUnit']").length>0 &&  /^[ ]*$/.test($("input[type='hidden'][name='receiveFileSendFileUnit']").val())){
        whir_alert("来文单位不能为空！",function(){$("input[type='hidden'][name='receiveFileSendFileUnit']").focus();});
    ...
    //判断转收文，文号是否重复
    if($("input[type='text'][name='receiveFileFileNumber']").length>0 && $("input[type='text'][name='receiveFileFileNumber']").val()!=""){
        var receiveFileFileNumberOld='';
        if($("#receiveFileFileNumberOld").length>0)
            receiveFileFileNumberOld=$("#receiveFileFileNumberOld").val();
        var sfcfflag=true;
        var receiveWh=$("input[type='text'][name='receiveFileFileNumber']").val();
        var receiveWH2=Base64.encode(receiveWh);
        if(receiveWh!=receiveFileFileNumberOld){
        //设置post同步
        $.ajaxSetup({
            async: false
        });
        $.post(
            "GovDocReceiveProcess!receiveWhsfcf.action",
            {
               receiveWh:receiveWH2
            },
            function(data)
            {
             var dataObj=eval("("+data+")");//转换为json对象
             var flagWh=dataObj.data.sfcf;
             if(flagWh=="true"){
                 var TsContent=receiveWh+"文号您已登记过，请确认是否继续登记?";
                 if(window.confirm(TsContent)){
	...
	if($("*[name='isChangeSeq']").length>0 && $("*[name='isChangeSeq']").val() == 1){
...
function cmdSaveDraft(){
    ...
    if(!initPara()) return;
    $('#dataForm').attr("action","/defaultroot/GovDocReceiveProcess!saveDraft.action");
...
function saveDraftOk(){
    whir_alert("保存成功！",function(){
		try{
			if( window.opener && window.opener.document.getElementById("queryForm") ){	window.opener.refreshListForm_("queryForm");} window.close();
		}catch(e){
			window.close();
		}
	});
