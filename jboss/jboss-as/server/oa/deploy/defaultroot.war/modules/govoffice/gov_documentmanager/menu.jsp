...
%>
    ...
    <SCRIPT type="text/javascript">  
		var zNodes = [
			...
			,{ id:10000000032, pId:1000000003, name:"已登记收文", url:"<%=rootPath%>/GovRecvDocSet!isHandle.action", target:'mainFrame',iconSkin:"fa fa"}
			,{ id:10000000033, pId:1000000003, name:"已转本部门收文", url:"<%=rootPath%>/GovRecvDocSet!isTomyDepart.action", target:'mainFrame',iconSkin:"fa fa"}
			,{ id:10000000034, pId:1000000003, name:"无需处理收文", url:"<%=rootPath%>/GovRecvDocSet!notHandle.action", target:'mainFrame',iconSkin:"fa fa"}
			</c:if>
			<%
				menuIndex++;
			...
			<c:if test="${fn:contains(canShowMenus,'documentmanager_sendfile')}">
			 ...
			 <%if(displaysw){%>
			 	{ id:-2, pId:-1, name:"发文管理",iconSkin:"fa fa-cog fa"}
			 <%}%>
			,{ id:1100000004, pId:-2, name:"新建发文", click:"", target:'_blank',iconSkin:"fa fa"}
			<%
			...
			%>
			,{ id:<%=processId%>, pId:1100000004, name:"<%=processName%>",click:"openWin({url:'<%=rootPath%>/GovDocSendProcess!newDoc.action?p_wf_pool_processId=<%=processId%>&p_wf_processType=<%=processType%>',isFull:'false',winName:'govworkflow'})" ,url:'',iconSkin:"fa fa"}
			<%
				 }
			}
			%>
			...
			<%if(null != request.getAttribute("hasHandOutFlag") && "true".equals(request.getAttribute("hasHandOutFlag")+"")){%>
			...
			,{ id:1200000012, pId:-3, name:"新建收文", url:"", target:'_blank',iconSkin:"fa fa"}
			<%
			 ...
			 %>
					,{ id:<%=processId%>, pId:1200000012, name:"<%=processName.replaceAll("\"","&quto;")%>" ,click:"openWin({url:'<%=rootPath%>/GovDocReceiveProcess!newDoc.action?p_wf_pool_processId=<%=processId%>&p_wf_processType=<%=processType%>',isFull:'false',winName:'govrecworkflow'})" ,url:'',iconSkin:"fa fa"}
			 <%
				 }
			%>
					,{ id:1100000114, pId:-3, name:"草稿箱", url:"<%=rootPath%>/GovDocReceiveProcess!receiveListDraft.action", target:'mainFrame',iconSkin:"fa fa"}
			...
			,{ id:1000000017, pId:-4, name:"新建送审签", url:"", target:'mainFrame',iconSkin:"fa fa"}
				...
				%>
            ,{ id:1100000118, pId:-4, name:"草稿箱", url:"<%=rootPath%>/GovDocSendCheckProcess!sendCheckListDraft.action", target:'mainFrame',iconSkin:"fa fa"}
            ,{ id:1000000119, pId:-4, name:"经办文件查阅", url:"<%=rootPath%>/GovDocSendCheckProcess!handlingFileList.action", target:'mainFrame',iconSkin:"fa fa"}
			...
			,{ id:1000000025, pId:-6, name:"阅读情况统计", url:"<%=rootPath%>/GovDocSet!readStatisticsDoc.action?govtype=receivefile", target:'mainFrame',iconSkin:"fa fa"}
			<%} }%>
			<%menuIndex++;if(com.whir.common.util.CommonUtils.isForbiddenPad(request)){%>
