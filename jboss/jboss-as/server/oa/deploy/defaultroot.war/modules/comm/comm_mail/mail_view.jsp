...
<%
   ...
   **/
   ...
   String frommod = com.whir.component.security.crypto.EncryptUtil.htmlcode(request.getParameter("frommod"));
   if(null!= frommod && !"".equals(frommod) && !"null".equals(frommod)){
		frommod = com.whir.component.security.crypto.EncryptUtil.replaceHtmlcode(frommod);
   }

   String portletSettingId = com.whir.component.security.crypto.EncryptUtil.htmlcode(request.getParameter("portletSettingId"));
   if(null!= portletSettingId && !"".equals(portletSettingId) && !"null".equals(portletSettingId)){
		portletSettingId = com.whir.component.security.crypto.EncryptUtil.replaceHtmlcode(portletSettingId);
   }
	
   String boxId = com.whir.component.security.crypto.EncryptUtil.htmlcode(request.getParameter("boxId"));
   if(null!= boxId && !"".equals(boxId) && !"null".equals(boxId)){
		boxId = com.whir.component.security.crypto.EncryptUtil.replaceHtmlcode(boxId);
   }
			...
			<tr>
				...
				<div style="padding-left:15px;" >
				   ...
				   %>
					...
					<%}%>
					<%if(index<recordCount-1&&!"notreadmaillist".equals(frommod)){%>
					<a href="#" style="color:#000;" onclick="goNextMail('<%=index+1%>');" class="but_class">
					   <span class="but_fontspce"><%=Resource.getValue(whir_locale,"mail","mail.nextMail")%></span><span class="but_rightbg"></span>
					</a>
					<%}%>
					<%if(index<recordCount-1&&"notreadmaillist".equals(frommod)){//未读邮箱刷新列表  防止点击上一封或者下一封出现跳跃读取，next不加1%>
					<a href="#" style="color:#000;" onclick="goNextMail('<%=index%>');" class="but_class">
					   <span class="but_fontspce"><%=Resource.getValue(whir_locale,"mail","mail.nextMail")%></span><span class="but_rightbg"></span>
					</a>
					<%}%>
				</div>
...
$(document).ready(function() {
	...
	<%}%>
	<%if("notreadmaillist".equals(frommod)){%>//未读邮箱刷新列表  防止点击上一封或者下一封出现跳跃读取
		Initialize();
	<%}%>
});
	function Initialize(){
    ...
    function goNextMail(index){
		...
		json = eval("("+json+")");
		if(json!=""){
			 	//alert('innerMail!viewMail.action?mailuserid='+json.mailuserid+'&mailid='+json.mailid+'&index='+json.index+'&recordCount='+recordCount+'&frommod=<%=frommod%>&boxId=<%=boxId%>');
            whir_tips("",1,'',function(){
	          location_href('innerMail!viewMail.action?mailuserid='+json.mailuserid+'&mailid='+json.mailid+'&index='+json.index+'&recordCount='+recordCount+'&frommod=<%=frommod%>&boxId=<%=boxId%>');
	        });
    ...
    function reflashandcloses(opeJson_,msg_json) {
		<%if("1".equals(fromdesktop)){%>
			try{
			opener.refreshMod('', '<%=portletSettingId%>');
		}catch(e){}
		<%}else{%>
          window.opener.refreshListForm_('queryForm');
		<%}%>
