...
String processId=request.getParameter("processId")==null?"":com.whir.component.security.crypto.EncryptUtil.htmlcode(request,"processId");
String draftParm=request.getParameter("p_wf_pool_processId")==null?"":com.whir.component.security.crypto.EncryptUtil.htmlcode(request,"p_wf_pool_processId");
if(draftParm!=null && !draftParm.equals("")&& !draftParm.equals("null")){
    draftParm="?p_wf_pool_processId="+draftParm;
}
...
<body  onload="initBody();" scroll=no class="docBodyStyle">
    ...
    <s:form name="dataForm" id="dataForm" action="${ctx}/kw!save.action" method="post" theme="simple" >
     ...
     <!--  <input type="button" class="btnButton4font" onClick="ok2(0,this);" value="保存退出" />
      <input type="button" class="btnButton4font" onClick="cmdSave();" value="保存草稿" /> -->
     </s:form>
	</div>
    ...
    var validation = validateForm("dataForm");
     if(validation){
      ...
      if(val != null && val !='' && val !='null'){
          ...
          var kwkind=whirCombobox.getText("_kwzl"); //$('#kwzl option[value='+val+']').text();
          $("#kwkind").val(kwkind);
