...
String fromEailTransmit=request.getParameter("fromEailTransmit")==null?"":com.whir.component.security.crypto.EncryptUtil.htmlcode(request,"fromEailTransmit");
...
<SCRIPT LANGUAGE="JavaScript">
    //打印
    function cmdPrint(){
         var val='<%=rootPath%>/modules/subsidiary/infosend/printWord.jsp';
        openWin({url:val,width:800,height:400,winName:'kwPrintWord'});
    }
    </SCRIPT>
</head>
                       ...
                       <div id="docinfo0" class="doc_Content">
							<!--表单包含页-->
                                <table width="100%" cellpadding="10" CELLSPACING="1" ID="receiveForm" class="receiveForm" align="center" class="Table_bottomline">
                                    <tr>
                                        ...
                                        <td width="25%">第
                                            ...
                                            <s:textfield name="kwPO.kwqs" id="kwqs" cssClass="inputText" whir-options="vtype:[{'maxLength':10},'spechar3']" cssStyle="width:20%" maxlength="10"/>
                                            <%} else {%>
                                    ...
                                    <tr>
                                        ...
                                        <td colspan="3">
                                           <%if(curModifyField.indexOf("$kwPO.yffs$") >= 0){%>
                                                 <s:textfield name="kwPO.yffs" id="yffs" cssClass="inputText" whir-options="vtype:[{'maxLength':6},'spechar3']"  cssStyle="width:97%" maxlength="6"/>
                                             <%} else {%>
                                                  ...
                                                  for(int i = 0; i < xxList.size(); i ++) {
                                                    ...
                                                    <tr>
                                                      <td>
                                                        ...
                                                        <input type="checkbox" name="xxid"  value="<%=obj[0]%>" >
														<%}%>
                                    ...
                                    <tr>
                                        ...
                                        <td colspan="3">
                                            <%if(curModifyField.indexOf("$kwPO.kwzw$") >= 0){%>
                                              <input type="button" class="btnButton4font" value="编辑正文" onclick="bjzw('edit');" />
                                              <%} else {%>
                                ...
                                <table width="100%">
                                    <tr>
                                        <td>
                                         <%if(!fromEailTransmit.equals("1")){%>
                                         <input type="button" class="btnButton4font" value="信息采用" onclick="xxcy();" />
                                         <%}%>
...
function bjzw(type) {
    ...
    if(type == "edit") {
        ...
        document.getElementById("Template").value = val.split("_")[1];
        ...
        $("#CanSave").val("0");
        document.getElementById("iframe1").src = "<%=rootPath%>/modules/subsidiary/infosend/ckzw.jsp?kwid=" + kwid+"&type="+type;
    }
}
...
function initPara() {
    ...
    if(validation){
        ...
        if(obj.tagName == "SELECT") {
            ...
            var kwkind=$('#_kwzl option[value='+val+']').text();
            $("#kwkind").val(kwkind);
            return true;
        }
    }
...
function xxcy() {
    var id =getCheckBoxData("xxid", "value");
    var kwName=$("#kwzlmc").val();
    if(id != null && id !=""){
        var url='${ctx}/jfbz!list.action?op=select&kwName='+kwName+'&xxid='+id;
        //alert("id:"+id);
        popup({id:'selcyqk1',content: 'url:'+url,title:'积分标准',width: '500px',height: '400px',lock: true,resize: false}); 
        
        //openWin({url:url,width:600,height:480,winName:'xxcy'});
    }else{
        whir_alert("请选择初选信息！",null,null);
    }
}
</script>
</html>