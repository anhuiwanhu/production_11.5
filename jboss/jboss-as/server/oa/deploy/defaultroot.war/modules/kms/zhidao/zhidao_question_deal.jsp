...
<div class="wh-wrapper">
  <s:form name="dataForm" id="dataForm" action="" method="post" theme="simple" >
	...
	<input type="hidden" name="answerAskAgainContent" id="answerAskAgainContent" value=""/><!--追问内容-->
	<input type="hidden" name="questionGoodComment" id="questionGoodComment" value=""/><!--满意回答评价-->
                            ...
                            <div class="wh-q2a-answer wh-q2a-ans-best">                          
                                <div class="wh-q2a-tip">
                                    <div class="wh-q2a-tip-btn-two">
									    <a id="askGoonBtn1" href="###" onclick="askAgin('<%=questionGoodAnswerId%>');"><i class="fa fa-my-reply"></i>追问</a>
								...
								<%}%>
								<div class="wh-q2a-a-comipt" id="askAgin_<%=questionGoodAnswerId%>" style="display:none">
									<input type="text" id="askAgainContent_<%=questionGoodAnswerId%>" name="askAgainContent"/>
									<a href="###" class="a-com-btn" onclick="saveAskAgainContent('<%=questionGoodAnswerId%>',this);">提交问题</a>
								</div>
                            ...
                            <div class="wh-q2a-answer wh-q2a-ans-reply">
                                ...
                                <div class="wh-q2a-tip">
								    <div class="wh-q2a-tip-btn">
                                        <a id="askGoonBtn2" href="###" onclick="setGoodAnswer('<%=answerId%>');"><i class="fa fa-my-reply"></i>选为满意答案</a>
                                    </div>
								    <div class="wh-q2a-tip-btn">
                                        <a id="askGoonBtn2" href="###" onclick="askAgin('<%=answerId%>');"><i class="fa fa-my-reply"></i>追问</a>
                                    </div>
                                    <div class="wh-q2a-tip-vote">
								...
								<%}%>
								<div class="wh-q2a-a-comipt" id="goodAnswer_<%=answerId%>" style="display:none">
									<input type="text" id="goodAnswerContent_<%=answerId%>" name="goodAnswerContent"/>
									<a href="###" class="a-com-btn" onclick="saveGoodAnswer('<%=answerId%>',this);">提交评价</a>
								</div>
								<div class="wh-q2a-a-comipt" id="askAgin_<%=answerId%>" style="display:none">
									<input type="text" id="askAgainContent_<%=answerId%>" name="askAgainContent"/>
									<a href="###" class="a-com-btn" onclick="saveAskAgainContent('<%=answerId%>',this);">提交问题</a>
								</div>
...
<script type="text/javascript">
	...
	function clickAnswerComment(answerId){
		...
		//选为满意回答
	function setGoodAnswer(answerId){
		if($("#goodAnswer_"+answerId).is(":hidden")){
			$("#goodAnswer_"+answerId).show();
		}else{
			$("#goodAnswer_"+answerId).hide();
		}
	}
	
	//保存满意答案
	function saveGoodAnswer(answerId,obj){
		var questionGoodComment =$("#goodAnswerContent_"+answerId).val();
		if(questionGoodComment ==''){
			whir_alert('请输入评价内容！',function(){
				$("#goodAnswerContent_"+answerId).focus();
			});
		}else{
			if(questionGoodComment.length >500){
				whir_alert('评价内容已超过500字，请重新输入！',function(){
					$("#goodAnswerContent_"+answerId).focus();
				});
			}else{
				if(checkIllegalCharacter(questionGoodComment,"评价内容")){
					return false;
				}
				$("#answerId").val(answerId);
				$("#questionGoodComment").val(questionGoodComment);
				$('#dataForm').attr("action","/defaultroot/question!setGoodAnswer.action");
				ok(1,obj);
			}
		}
	}
	...
	//继续追问
	function askAgin(answerId){
		if($("#askAgin_"+answerId).is(":hidden")){
			$("#askAgin_"+answerId).show();
		}else{
			$("#askAgin_"+answerId).hide();
		}
	}
	
	//保存追问
	function saveAskAgainContent(answerId,obj){
		var askAgainContent =$("#askAgainContent_"+answerId).val();
		if(askAgainContent ==''){
			whir_alert('请输入追问内容！',function(){
				$("#askAgainContent_"+answerId).focus();
			});
		}else{
			if(askAgainContent.length > 1000){
				whir_alert('追问内容已超过1000字，请重新输入！',function(){
					$("#askAgainContent_"+answerId).focus();
				});
			}else{
				if(checkIllegalCharacter(askAgainContent,"追问内容")){
					return false;
				}
				$("#answerId").val(answerId);
				$("#answerAskAgainContent").val(askAgainContent);
				$('#dataForm').attr("action","/defaultroot/question!saveAskAgainContent.action");
				ok(1,obj);
			}
		}
	}
